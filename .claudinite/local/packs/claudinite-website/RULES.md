...
- **`mcp__github__search_issues`/`list_issues` without a narrowed `fields` list can
  blow the MCP token limit — this repo's tracker issues (`Claudinite tracker:
  <dimension>`) accumulate long comment histories, so an unnarrowed call over them
  returns every match's full body and errors before it returns anything.**
  `search_issues` is natural-language semantic matching over the whole issue, not a
  raw GitHub `in:title`-style qualifier search — a query ending in `in:title` still
  surfaces issues whose titles don't contain the term, so appending it buys no
  narrowing. Pass `fields: ["number", "title", "state"]` (or similarly narrow) on
  the *first* call whenever you only need to locate an issue, not after the
  "exceeds maximum allowed tokens" error forces a retry. Hit independently in three
  separate scheduled runs (issues #159, #161, #168), each paying a wasted round-trip
  (and, twice, a manual dump-and-reparse) for a result the narrowed call answers
  cleanly.

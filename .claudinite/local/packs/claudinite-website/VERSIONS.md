...
| Date | Task | Change |
|---|---|---|
| 2026-08-30 | `prose-to-checks-sweep` | Converted the checkable half of RULES.md's self-description rule — that `site/README.md`'s claim about the deploy workflow's push trigger carrying no `paths:` filter must match `.github/workflows/deploy-pages.yml` (the #45 case) — into check `claudinite-website/deploy-trigger-claim`. RULES.md's own paragraph carries a broader discipline (correct *every* self-description a behaviour change falsifies) the check can't cover and stays as prose. |

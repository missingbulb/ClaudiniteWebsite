...
// The gerund phrasing this repo's product-wiki actually uses ("on pushes to
// `main` touching `site/**`") — no "that", so it only clears QUALIFIER because
// of the bare `touching` alternative.
const GERUND_CLAIM = 'Deploys ride GitHub Pages on pushes to `main` touching `site/**`.\n';
...
test('fires on the gerund phrasing ("touching", no "that")', () => {
  const found = rule.run(ctxWith({ md: { 'product-wiki/Website/README.md': GERUND_CLAIM } }));
  assert.equal(found.length, 1);
  assert.equal(found[0].file, 'product-wiki/Website/README.md');
});
...
test('stays quiet on an honest "no path filter" claim — no `paths:` colon, no touch/only wording', () => {
  const md = { 'site/README.md': 'Published on every push to `main`. The workflow\'s push trigger carries no path filter, since the version bump it commits back has to run on every release.\n' };
...
test('claims() picks up the gerund phrasing too', () => {
  const found = claims(GERUND_CLAIM);
  assert.equal(found.length, 1);
  assert.deepEqual(found[0].globs, ['site/**']);
});

// The release's gate: has `main` moved since the last release went out?
//
// This is the task's own term because no built-in asks the question a release asks.
// `substantive-change` excludes task-authored commits, so a landed `site-stats`
// recount — a real change to what the site serves — would read as silence and never
...
// `[skip ci]` used to be, stated as a fact about the branch rather than as a flag on a
// commit message — nothing can accidentally re-arm a release by rewording a commit.
...
// own `CLAUDINITE_PACK`/`CLAUDINITE_TASK`; `preconditions.test.mjs` holds this literal
// against the two directory names those are derived from.
export const RELEASE_TASK_ID = 'claudinite-website/site-release';
